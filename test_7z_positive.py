...
def test_step0():
    res = []
    upload_files(data["host"],
                 data["user"],
                 "12345",
                 "{}/p7zip-full.deb".format(data["local_path"]),
                 "{}/p7zip-full.deb".format(data["remote_path"]))
    res.append(ssh_checkout("0.0.0.0",
                            "user2",
                            "12345",
...
def test_step1(make_folders, clear_folders, make_files, home_task1):
    # test1
    res1 = ssh_checkout(data["host"],
                        data["user"],
                        "12345",
                        "cd {}; 7z a {}/arx1.7z ".format(data["folder_in"],
                                                         data["folder_out"]),
                        "Everything is Ok"), "Test1 Fail"
    res2 = ssh_checkout(data["host"],
                        data["user"],
                        "12345",
...
def test_step2(clear_folders, make_files, home_task1):
    # test2
    res = []
    res.append(ssh_checkout(data["host"],
                            data["user"],
                            "12345",
                            "cd {}; 7z a {}/arx1.7z"
                            .format(data["folder_in"],
                                    data["folder_out"]), "Everything is Ok"))
    res.append(ssh_checkout(data["host"],
                            data["user"],
                            "12345",
                            "cd {}; 7z e arx1.7z -o{} -y"
                            .format(data["folder_out"],
